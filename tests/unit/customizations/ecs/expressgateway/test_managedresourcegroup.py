...
class TestManagedResourceGroup:
    ...
    def test_create_key(self, resource1):
        group = ManagedResourceGroup()
        key = group._create_key(resource1)
        assert key == "LoadBalancer/lb-123"

    def test_get_status_string_empty_group(self):
        group = ManagedResourceGroup(resource_type="EmptyGroup", resources=[])
        status_string = group.get_status_string("⠋")
        assert "EmptyGroup" in status_string
        assert "<empty>" in status_string

    def test_combine_resource_groups(self, resource1, resource2):
        group1 = ManagedResourceGroup(resources=[resource1])
        group2 = ManagedResourceGroup(resources=[resource2])
        combined = group1.combine(group2)
        assert len(combined.resource_mapping) == 2
    ...
    def test_combine_overlapping_resources(self):
        ...
        assert key in combined.resource_mapping
        assert combined.resource_mapping[key].status == "ACTIVE"
